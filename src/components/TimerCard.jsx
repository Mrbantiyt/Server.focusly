...
// Plays a short beep using the Web Audio API — no audio file/asset needed,
// and it works even if the tab has no other audio permissions requested.
function playBeep() {
  try {
    ...
    playTone(now, 880);
    playTone(now + 0.45, 880);
    playTone(now + 0.9, 1175);
...
export function TimerCard({
  ...
  // Beep + alert exactly once per completion (not on every re-render while `finished` stays true)
  useEffect(() => {
    if (finished && !beepedRef.current) {
      beepedRef.current = true;
      playBeep();
    }
    if (!finished) beepedRef.current = false;
  }, [finished]);
  ...
  const handleReset = () => {
    if (remaining !== durationSeconds && !window.confirm("Reset the timer back to the set duration? (Time today already saved is unaffected.)")) return;
    onReset?.();
  };
