...
export function useTasks(uid) {
  ...
  // recomputed real elapsed time to Firestore every ~5s. Because the value
  // is derived from `startedAt` rather than accumulated tick-by-tick, it's
  // automatically correct even after the interval was throttled/suspended
  // (screen off, app backgrounded) — the next time it fires (e.g. right
  // when the screen turns back on) it just computes the real gap.
  useEffect(() => {
    if (!uid) return;
